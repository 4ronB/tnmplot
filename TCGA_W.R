#this function servers to compare expression values of paired normal and tumor RNA-Seq samples from GDC datasets using Wilcoxon test

paired_tcga <- function(tissueID, geneName){
  ...
  sampNames <- usedTissue[!is.na(usedTissue$Pair),]#screening for paired data
  sampNames <- sampNames[!sampNames$Pair == "ismetlodo case de nincs par" ,]
  sampNames <- sampNames[!sampNames$Meta == 1,]
  norms <-  as.vector(unlist(sampNames$Pair))
  normSamps <- usedTissue[usedTissue$`Sample ID`%in% norms,]
  sampNames <- merge(sampNames, normSamps, all = T)
  sampNames$type <- NA
  ...
  #merging the clinical data and expression table - this data fram will be used for further investigations
  sampExp <- sampNames
  ...
  wt <- wilcox.test(x = sampExp[sampExp$type == "Normal",3],
                    y = sampExp[sampExp$type == "Tumor",3],
                    paired = T, exact = F, conf.int = T)
  pvalue <- format(wt$p.value, digits = 3, scientific = T)
